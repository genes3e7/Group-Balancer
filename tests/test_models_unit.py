...
def test_solver_config_validation_errors():
    ...
    with pytest.raises(ValueError, match="Number of groups must be positive"):
        SolverConfig(num_groups=0, group_capacities=valid_caps, score_weights=valid_weights)
    with pytest.raises(ValueError, match="Number of groups exceeds limit"):
        SolverConfig(num_groups=1000, group_capacities=valid_caps, score_weights=valid_weights)
    ...
    with pytest.raises(ValueError, match="exceeds limit of"):
        SolverConfig(num_groups=1, group_capacities=[10000], score_weights=valid_weights)
    ...
    with pytest.raises(ValueError, match="cannot be negative"):
        SolverConfig(num_groups=2, group_capacities=[5, -1], score_weights=valid_weights)
    ...
    with pytest.raises(ValueError, match="must be a finite number"):
        SolverConfig(num_groups=1, group_capacities=[1], score_weights={"S1": float('inf')})
    with pytest.raises(ValueError, match="cannot be negative"):
    ...
    with pytest.raises(ValueError, match="grouper_weight must be a finite number"):
        SolverConfig(num_groups=1, group_capacities=[1], score_weights=valid_weights, grouper_weight=float('nan'))
    with pytest.raises(ValueError, match="grouper_weight cannot be negative"):
        SolverConfig(num_groups=1, group_capacities=[1], score_weights=valid_weights, grouper_weight=-1)
    with pytest.raises(ValueError, match="grouper_weight exceeds safe limit"):
        SolverConfig(num_groups=1, group_capacities=[1], score_weights=valid_weights, grouper_weight=2_000_000)
